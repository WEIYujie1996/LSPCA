function  [Z, L, B, K] = iklrpca(X, Y, gamma, sigma, k, Linit, Kinit)
% Inputs:
...
while notConverged
    ...
    problem.M = manifold;
    problem.cost  = @(Ltilde) cost_fun(Ltilde, B, B0, X, Ymask, Xnorm, n, gamma);
    problem.egrad = @(Ltilde) Lgrad(Ltilde, B, B0, X, Y, Xnorm, numClasses, n, p, k,  gamma);
    options.verbosity = 0;
