function [Zu, Yu, Zl, L, B, K] = klrpca_gamma_lambda_ss(Xu, Xl, Yl, lambda, gamma, sigma, k, Linit, Kinit)
...
while notConverged
    ...
    problem.egrad = @(Ltilde) Lgrad(Ltilde, B, B0, X, Xl, Yl, Xnorm, numClasses, n, nl, p, k, gamma, lambda);
    options.verbosity = 0;
...
function g = Lgrad(L, B, B0, X, Xl, Yl, Xnorm, numClasses, n, nl, p, k, gamma, lambda)
g = zeros(n,k);
for j = 1:numClasses
    Xj = Xl(Yl==j, :);
    bj = B(:,j);
