function  [Zu, Yu, Zl, L, B, K] = iklrpca_ss(Xu, Xl, Yl, gamma, sigma, k, Linit, Kinit)
...
[~, Yu] = max(mnrval(B,Zu), [], 2); %evaluate via LR for ease, equivalent to softmax B being returned
B = [B, zeros(k+1,1)];
K = X;
end
