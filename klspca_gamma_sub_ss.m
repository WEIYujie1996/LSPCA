function [Zu, Yu, Zl, L, B, K] = klspca_gamma_sub_ss(Xu, Xl, Yl, lambda, gamma, sigma, k, L0, Kinit)
...
problem.M = manifold;
problem.cost  = @(L) (1/Xnorm^2)*lambda*norm(X - gamma^-1*X*L*L', 'fro')^2 + (1-lambda)*(1/Ynorm^2)*norm(Yl - (Xl*L)*(pinv(Xl*L)*Yl), 'fro')^2;
problem.egrad = @(L) (2*(1/Xnorm^2)*lambda*(gamma^-2 - 2/gamma)*((L'*X')*X) - 2*(1-lambda)*(1/Ynorm^2)*(pinv(Xl*L)*Yl)*((Yl'*(eye(nl)-(Xl*L)*pinv(Xl*L))*Xl)))';
options.verbosity = 0;
